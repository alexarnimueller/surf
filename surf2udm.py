...
def surf2ord(
    input_file: Annotated[
        str, typer.Argument(help="name of the input file in SURF format")
    ],
    output_file: Annotated[
        str, typer.Argument(help="name of the output file in XML format; suffix: .xml")
    ],
    delimiter: Annotated[
        str, typer.Argument(help="delimiter of the input file")
    ] = "\t",
):
    # read the SURF file
    ...
    if df["product_1_yield"].max() <= 3.0:
        logger.warning(
            "Yields seem to be provided as fractions instead of percentages. Multiplying by 100"
        )
        for c in df.columns:
            if c.endswith("yield"):
                df[c] = df[c] * 100.0
    ...
    version = ET.SubElement(
        udm, "UDM_VERSION", MAJOR="6", MINOR="0", REVISION="0", VERSIONTEXT="6.0.0"
    )
    ...
    for idx, row in track(
        df.iterrows(), total=len(df), description="Reading molecules..."
    ):
        row = row.dropna()
        err = False
        ...
        for cpd in sorted(set(re.findall("\w+_[1-9]", " ".join(row.index.tolist())))):
            ...
            if mol:
                ...
                if cas:
                    cas = re.sub(
                        r"-0([0-9])$", r"-\g<1>", cas
                    )  # replace potential leading zero for last CAS pos
                    mol_dict[MolToInchiKey(mol)].update({"CAS": cas})
            else:
                logger.error(
                    f"Can't process SMILES/InChI of {cpd} in reaction {row.rxn_id}"
                )
                err = True
                continue

        # reading the provenance / citations
        if "source_id" in row and re.match(
            doi_pattern, row.source_id
        ):  # catch DOI if present
            ...
            if doi not in cit_dict:
                ...
                #     ET.SubElement(cit, "TYPE").text = row["source_type"]
                cnt_cit += 1
        ...
        if err:
            ...
            logger.error(
                f"Couln't process all SMILES in reaction {row.rxn_id}, skipping reaction"
            )

    # processing the individual, unique compounds
    for k, v in track(
        mol_dict.items(), total=len(mol_dict), description="Adding molecules to UDM..."
    ):
        mol = ET.SubElement(molecules, "MOLECULE", ID=k)
        ET.SubElement(mol, "MOLSTRUCTURE").text = (
            "<![CDATA[" + v["MOLSTRUCTURE"] + "]]>"
        )
        if "CAS" in v:
    ...
    if invalid:
        logger.warning(
            f"Ignoring {len(invalid)} reactions where SMILES could not be read for one of the components"
        )
        df = df[~df.rxn_id.isin(invalid)]  # exclude invalid reactions

    # processing the individual reactions
    for idx, row in track(
        df.iterrows(), total=len(df), description="Processing reactions..."
    ):
        if "rxn_id" not in row:
            logger.error(f"Reaction ID missing! Can't process reaction {idx}")
            continue
        ...
        if "atmosphere" in row:
            atmo = ET.SubElement(conditions, "ATMOSPHERE")
            atmo.text = row.atmosphere
        ...
        for cpd in sorted(set(re.findall("\w+_[1-9]", " ".join(row.index.tolist())))):
            if (
                ...
                logger.error(
                    f"no SMILES, InChI, CAS or Name forund for {cpd} in {row.rxn_id}, skipping reaction"
                )
                err = True
                continue
            ...
            cpd_name = row.get(
                ...
                row.get(
                    f"{cpd}_name",
                    row.get(f"{cpd}_smiles", row.get(f"{cpd}_inchi", "unknown")),
                ),
            )
            ...
            if f"{cpd}_smiles" in row:
                ...
                logger.error(
                    f"no SMILES/InChI forund for {cpd} in {row.rxn_id}, skipping reaction"
                )
                err = True
                continue
            ...
            if "product" in cpd:
                ...
                if f"{cpd}_eq" in row and "scale_mol" in row and row.scale_mol > 0:
                    ...
                    rct_amnt.text = (
                        f"{row.scale_mol * row[f'{cpd}_eq'] * 1000:.4f} mmol"  # mmol
                    )
            else:
                if "solvent" in cpd:
                    ...
                    if (
                        ...
                        try:
                            ...
                            if (
                                f"{cpd}_fraction" in row
                            ):  # check if fraction available (multiple solvents present)
                                solv_amnt.text = f'{row[f"{cpd}_fraction"] * scale / conc * 1000:.4f} ml'  # ml
                            else:
                                solv_amnt.text = f"{scale / conc * 1000:.4f} ml"  # ml
                        except ValueError:
                            logger.warning(
                                f"Can't process {cpd}_fraction to amount in reaction {row.rxn_id}"
                            )
